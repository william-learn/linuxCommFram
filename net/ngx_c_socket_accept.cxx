...
void CSocekt::ngx_event_accept(lpngx_connection_t oldc)
{
    ...
    {     
        ...
        //accept系统调用已经不存在惊群了，在父进程中bind,listen，然后fork出子进程，
        //所有的子进程都accept这个监听句柄。这样，当新连接过来时，仅有一个子进程返回新建的连接，其他子进程继续休眠在accept调用上，没有被唤醒。
        
        if(s == -1)
        {
            ngx_log_error_core(8, 0,"惊群测试:ngx_event_accept()中accept失败,进程id=%d",ngx_pid); 
        }
        else
        {
            ngx_log_error_core(8, 0,"惊群测试:ngx_event_accept()中accept成功,进程id=%d",ngx_pid); 
        }     
        ...
        // 这里我们并不添加EPOLLOUT事件，因为采取的是EPOLLLT模式        
