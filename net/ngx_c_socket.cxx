...
//--------------------------------------------------------------------
//(1)epoll功能初始化，子进程中进行（意思是创建了4个epoll红黑树） ，本函数被ngx_worker_process_init()所调用
                        ...
                        )
{
    ...
    if(eventtype == EPOLL_CTL_ADD) //往红黑树中增加节点；
    {
        ...
        // 加上 EPOLLEXCLUSIVE 解决惊群问题
        ev.events = flag;
        ev.events |= EPOLLEXCLUSIVE;      //既然是增加节点，则不管原来是啥标记
        pConn->events = flag;  //这个连接本身也记录这个标记
        pConn->events |= EPOLLEXCLUSIVE;
    }
...
{   
    ...
    // 但是只有一个worker进程 调用accept/accept4成功！！
    // 惊群的原因是因为 有4个epoll红黑树上都有监听的端口
    ngx_log_error_core(8, 0,"惊群测试:events=%d,进程id=%d",events,ngx_pid); 
    ngx_log_error_core(8, 0,"----------------------------------------");
        ...
        if(pSocketObj->m_iSendMsgQueueCount > 0) //原子的 
        {
            ...
            while(pos != posend)
            {
                ...
                //这里是重点，我们采用 epoll水平触发的策略，能走到这里的，都应该是还没有投递 写事件 到epoll中
	                ...
	                //如果返回了EAGAIN【发送缓冲区满了，需要等待可写事件才能继续往缓冲区里写数据】，此时，我再把写事件通知加入到epoll，
